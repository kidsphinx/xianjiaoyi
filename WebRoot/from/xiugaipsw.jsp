...
<body>
    ...
    <div class="register">
        <form id="bdyz" action="Frongeren" method="post">
            <div class="tel clearfix">
            	<span>旧密码</span>
            	<input type="password"  id="oripwd" onblur="checkuser()"/>    
            	<i id="m1"></i>        	                                                   
            </div>
            <div class="tel clearfix">
                 <span>新密码</span>
                <input type="password"  id="pwd1" readonly="readonly">
                
            </div>
            <div class="tel clearfix">
            	<span>确认密码</span>
            <input type="password"  id="pwd2" readonly="readonly" onblur="checkSamePwd()"/> 
            </div>
            <div class="tel clearfix">
                <span></span>
                <input type="button" value="确认修改" class="sub" id="sub" onclick="xiugaipsw()" disabled="disabled">
            </div>
        </form>
	...
	function xiugaipsw(){
		...
		$.ajax({
			...
			success:function(m){
				if(m=='ok'){
					parent.location.href="from/login.jsp";
				}
				else
				{
					alert("更改失败")
